	...
	*new {|path, preset, autopreset|
	...
	init  {|apath, preset, autopreset|
		...
		synthdef = SynthDef(\feed, {|in=2, out=0, loop=10, gainin=0, feedback=0.02, deltime=75, revtimes=5,
			amp=0.6, damping=1360, mod=1, vol=0.9, chord=#[ 40, 47, 52, 55, 59, 64 ], on=1|
...
/*		controls[\play] = Button(w, 18@18)
		...
		.action_({ arg butt;
			if (notchsynth.isNil, {
				...
				notchsynth = Synth.after(synth, \autonotch,
		...
		controls[\feedback] = EZSlider( w,         // parent
			slbounds,    // bounds
			"feedback",  // label
			ControlSpec(0, 0.5, \lin, 0.001, 0.02),     // controlSpec
		...
		controls[\amp] = EZSlider( w,         // parent
			slbounds,    // bounds
			"amp",  // label
			ControlSpec(0, 1, \lin, 0.001, 0.6),     // controlSpec
