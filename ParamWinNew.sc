...
ParamWinNew {
	...
	*new {|globalvarStr, aControlSpec, aWidget, width=600, height=240, presetspath|
	...
	initParamWin {|globalvarStr, aControlSpec, aWidget, width, height, apath|
		...
		lloopf = Task {
			inf.do({//arg index;
				if (playing, {
					...
					//if ( varStr != "", { try{ (varStr+"="+val).interpret } }); // here set value of the variable I control
		...
		Button(lwin,  Rect(270,height-60,80,25))
		.states_([
			["rand", Color.white, Color.black],
		])
		.action_({ arg butt;
			lmsl.value_(Array.fill(lwin.bounds.width / 6, {1.rand}));
		});
